...
function specialistNotice(st){
  if(st === "健康"){
    return "健康に関する表示は参考情報です。症状、治療、薬、診断に関する判断は医療機関へ相談してください。";
  }
  if(st === "お金"){
    return "金銭や投資に関する表示は参考情報です。投資判断、融資判断、契約判断は専門家へ相談してください。";
  }
  return "表示内容は参考情報です。重要な判断は占い結果のみで断定せず、必要に応じて専門家へ相談してください。";
}

function safeOpeningText(st, name){
  const base = {
    "恋愛": `${name}様の恋愛については、今の流れを参考として整理します。`,
    "復縁": `${name}様の復縁については、感情だけでなく関係の質も含めて参考整理します。`,
    "片思い": `${name}様の片思いについては、距離感と動く順番を参考として読み解きます。`,
    "仕事": `${name}様の仕事については、環境との相性と負担感を参考として見ていきます。`,
    "転職": `${name}様の転職については、準備状況と流れを参考として見ていきます。`,
    "独立": `${name}様の独立については、始め方と土台づくりを参考として整理します。`,
    "お金": `${name}様のお金の流れについては、参考情報として整え方を見ていきます。`,
    "人間関係": `${name}様の対人関係については、距離感と伝え方を参考として読みます。`,
    "結婚": `${name}様の結婚については、感情面と現実面の両方を参考として見ていきます。`,
    "家庭": `${name}様の家庭運については、役割分担と心の負担を参考として見ていきます。`,
    "健康": `${name}様の健康面については、生活の整え方を中心に参考情報として見ていきます。`,
    "総合運": `${name}様の全体の流れについて、参考情報として今の傾向を整理します。"
  };
  return base[st] || base["総合運"];
}
...
function generateReading(){
  const consentEl = $("consentCheck");
  if(consentEl && !consentEl.checked){
    alert("先に「運用確認」にチェックを入れてください。");
    return;
  }
  ...
  let html=`<div class="reportHeader"><div class="reportTitle">鑑定書</div><div class="reportSub">MOA PROFESSIONAL FORTUNE READING DOCUMENT</div></div><p><b>ご相談者：</b>${esc(name)}</p><p><b>相談テーマ：</b>${esc(topic)}</p><p><b>相談内容：</b><br>${esc(question).replace(/\n/g,"<br>")}</p><div class="callout">${esc(safeOpeningText(st, name))}</div><div class="callout">${esc(specialistNotice(st))}</div><div>${buildTags()}</div>`;
...
function bind(){
  ...
  $("resetBtn").addEventListener("click",()=>{
    ...
    $("topic").value="";$("gender").value="";if($("consentCheck")) $("consentCheck").checked=false;$("result").innerHTML=baseReportHeader();$("reportNumber").textContent="未発行";$("reportDate").textContent="未発行";$("memoView").textContent="未入力";$("proposalMemoView").textContent="未入力";
  });
